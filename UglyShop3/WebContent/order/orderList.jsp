      ...
      <tbody>
        <c:forEach var="orders" items="${orders}">
          <tr>
            ...
            <td><a href="<%= request.getContextPath() %>/order?cmd=view&id=<c:out value='${orders.orderID}'/>"><c:out value="${orders.prodName}" /></a></td>
            <td><c:out value="${orders.prodPrice * orders.orderQuantity}" /></td>
            <td><c:out value="${orders.orderQuantity}" /></td>
            <!-- 리뷰쓰기 버튼 -->
			<td><a class="btn btn-secondary mt-auto" href="<%= request.getContextPath() %>/review/reviewWrite.jsp?prodID=<c:out value="${orders.prodID}" />&prodName=<c:out value="${orders.prodName}" />&farmID=<c:out value="${orders.farmID}" />">리뷰쓰기</a></td>
