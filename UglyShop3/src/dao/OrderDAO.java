...
public class OrderDAO {
	...
	public boolean save(Order order) {
		...
		try {
			conn = dataSource.getConnection();
			pstmt = conn.prepareStatement("insert into orders (userID, prodID, prodName, prodPrice, orderQuantity, orderNum, farmID, is_status) values (?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public Order findByOrderId(int orderID) {
		Order order = null;
//		prodID, prodName, prodPrice, userID, userName, userAdd, userTel, orderQuantity, is_status, TrackNum
		try {
			conn = dataSource.getConnection();
			...
			if(rs.next()) {
				...
				closeAll();		//DB연결 객체들을 닫는 과정이 필요하다.
			}
