...
public class TodoDaoImpl implements TodoDao {
	...
	public void insertTodo(Todo todo) {
		...
		try {
			...
			pstmt.setDate(4, JDBCUtils.getSQLDate(todo.getTargetDate()));	// JDBCUtils에서 만들어둔 SQLDate메서드로 java날짜를 SQL날짜로 변경해 4번째 ?에 넣어줌
	...
	public Todo selectTodo(long todoId) {
		...
		try {
			...
			if(rs.next()) {	// 결과가 있으면 값을 저장. 없을때 저장하면 에러발생.
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();	// 그냥 rs.getDate("target_date")하면 java날짜와 SQL날짜형이 달라 에러나므로 로컬데이트(자바날짜)로 변환
